...
class CompletedPart {
  ...
  CompletedPart.fromXml(XmlElement? xml) {
    ...
    partNumber = int.tryParse(getProp(xml, 'PartNumber')?.text ?? "0");
  }
...
class CORSRule {
  ...
  CORSRule.fromXml(XmlElement? xml) {
    ...
    maxAgeSeconds = int.tryParse(getProp(xml, 'MaxAgeSeconds')?.text ?? "0");
  }
...
class DefaultRetention {
  ...
  DefaultRetention.fromXml(XmlElement? xml) {
    ...
    years = int.tryParse(getProp(xml, 'Years')?.text ?? "0");
  }
...
class NoncurrentVersionExpiration {
  ...
  NoncurrentVersionExpiration.fromXml(XmlElement? xml) {
    noncurrentDays = int.tryParse(getProp(xml, 'NoncurrentDays')?.text ?? "0");
  }
...
class Progress {
  ...
  Progress.fromXml(XmlElement? xml) {
    bytesProcessed = int.tryParse(getProp(xml, 'BytesProcessed')?.text ?? "0");
    bytesReturned = int.tryParse(getProp(xml, 'BytesReturned')?.text ?? "0");
    bytesScanned = int.tryParse(getProp(xml, 'BytesScanned')?.text ?? "0");
  }
...
class ScanRange {
  ...
  ScanRange.fromXml(XmlElement? xml) {
    end = int.tryParse(getProp(xml, 'End')?.text ?? "0");
    start = int.tryParse(getProp(xml, 'Start')?.text ?? "0");
  }
